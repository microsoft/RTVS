...
namespace Microsoft.UnitTests.Core.NSubstitute.Mef
{
    ...
    public class FactoryReflectionComposablePart : ComposablePart
    {
        ...
        {
            if (_composablePart.IsValueCreated || _imports == null)
            {
        ...
        private object[] GetArguments(Delegate valueFactory)
        {
            ...
            foreach (ImportDefinition ctorImportDefinition in ctorImportDefinitions)
            {
                ...
                {
                    throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, "There is no export for parameter of type {0}", parameterInfo.ParameterType));
                }
            }
