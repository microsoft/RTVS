    ...
    internal sealed class RmdPublishHandler : IMarkdownFlavorPublishHandler {
        ...
        private async Task RMarkdownRenderAsync(IRSession session, IFileSystem fs, string inputFilePath, string outputFilePath, string format, int codePage) {
            using (var fts = new FileTransferSession(session, fs)) {
                ...
                var publishResult = await session.EvaluateAsync($"rtvs:::rmarkdown_publish(blob_id = {rmd.Id}, output_format = {format.ToRStringLiteral()}, encoding = 'cp{codePage}')", REvaluationKind.RawResult);
