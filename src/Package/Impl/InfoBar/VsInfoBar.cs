...
namespace Microsoft.VisualStudio.R.Package.InfoBar {
    internal sealed class VsInfoBar : IInfoBar {
        private readonly Lazy<IVsInfoBarUIFactory> _factoryLazy = new Lazy<IVsInfoBarUIFactory>(() => VsAppShell.Current.GetGlobalService<IVsInfoBarUIFactory>(typeof(SVsInfoBarUIFactory)));
        private readonly IVsInfoBarHost _infoBarHost;

        public VsInfoBar(IVsInfoBarHost infoBarHost) {
