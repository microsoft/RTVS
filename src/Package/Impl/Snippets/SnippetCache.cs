...
namespace Microsoft.VisualStudio.R.Package.Snippets {
    public partial class SnippetListManager {
        ...
        internal class SnippetCache {
            ...
            private static Dictionary<string, Guid> ContentTypeToLanguageServiceGuid;
            internal static Dictionary<Guid, string> LanguageServiceGuidToContentType { get; private set; }
            private static Dictionary<Guid, List<ISnippetProvider>> SnippetProvidersPerLanguage;

            internal bool IsAbandoned { get; set; }

            static SnippetCache() {
                ContentTypeToLanguageServiceGuid = new Dictionary<string, Guid>(1, StringComparer.OrdinalIgnoreCase);
                ContentTypeToLanguageServiceGuid[RContentTypeDefinition.ContentType] = RGuidList.RLanguageServiceGuid;

                LanguageServiceGuidToContentType = new Dictionary<Guid, string>(1);
                LanguageServiceGuidToContentType[RGuidList.RLanguageServiceGuid] = RContentTypeDefinition.ContentType;

                SnippetProvidersPerLanguage = FindSnippetProvidersPerLanguage();
            }

            internal SnippetCache(IVsExpansionManager expansionManager, Guid languageServiceGuidToCache) {
                LanguageServiceGuid = languageServiceGuidToCache;
            ...
            internal void CacheLanguageExpansionStructs(IVsExpansionManager expansionManager) {
                ...
                Debug.Assert(_expansions == null, "The language has already been registered!");

                IVsExpansionEnumeration expansionEnumeration = null;
                List<VsExpansionEx> expansions = new List<VsExpansionEx>();
                ...
                try {
                    ...
                    while (VSConstants.S_OK == (hr = expansionEnumeration.Next(1, new IntPtr[] { handle.AddrOfPinnedObject() }, out fetched))) {
                        VsExpansion expansion = ConvertToVsExpansionAndFree((ExpansionBuffer)handle.Target);
            ...
            private void CacheExtensionSnippetInfo() {
                List<ISnippetProvider> snippetProviders = null;
                if (!SnippetProvidersPerLanguage.TryGetValue(LanguageServiceGuid, out snippetProviders))
                    return;

                string contentType = LanguageServiceGuidToContentType[LanguageServiceGuid];

                foreach (ISnippetProvider snippetProvider in snippetProviders) {
                    IEnumerable<XmlDocument> snippetDocuments = snippetProvider.GetSnippetDocuments(contentType);
                    CacheExtensionSnippetInfo(snippetDocuments);
                }
            }

            private void CacheExtensionSnippetInfo(IEnumerable<XmlDocument> snippetDocuments) {
                if (_expansions == null) {
                    return;
                }

                foreach (XmlDocument snippetDocument in snippetDocuments) {
                    _expansions.Add(new VsExpansionEx(snippetDocument));
                }
            }

            private static Dictionary<Guid, List<ISnippetProvider>> FindSnippetProvidersPerLanguage() {
                Dictionary<Guid, List<ISnippetProvider>> snippetProvidersPerLanguage = new Dictionary<Guid, List<ISnippetProvider>>();

                IEnumerable<Lazy<ISnippetProvider, IContentTypeMetadata>> snippetProviders =
                    ComponentLocatorWithMetadata<ISnippetProvider, IContentTypeMetadata>.ImportMany();

                foreach (Lazy<ISnippetProvider, IContentTypeMetadata> snippetProvider in snippetProviders) {
                    IEnumerable<string> contentTypes = snippetProvider.Metadata.ContentTypes;
                    foreach (string contentType in contentTypes) {
                        Guid languageServiceGuid;

                        if (!ContentTypeToLanguageServiceGuid.TryGetValue(contentType, out languageServiceGuid))
                            continue;

                        List<ISnippetProvider> snippetProvidersForCurrentContentType = null;
                        if (!snippetProvidersPerLanguage.TryGetValue(languageServiceGuid, out snippetProvidersForCurrentContentType)) {
                            snippetProvidersForCurrentContentType = new List<ISnippetProvider>(1);
                            snippetProvidersPerLanguage[languageServiceGuid] = snippetProvidersForCurrentContentType;
                        }

                        snippetProvidersForCurrentContentType.Add(snippetProvider.Value);
                    }
                }

                return snippetProvidersPerLanguage;
            }
            ...
            internal bool IsInitialized {
                get {
                    return _snippetInfoCache != null;
                }
            }

            void EnsureSnippetCache() {
                Debug.Assert(!IsInitialized, "Snippets are already loaded!");
                Debug.Assert(LanguageServiceGuid == HtmlGuids.HtmlLegacyLanguageServiceGuid ||
                             LanguageServiceGuid == CssGuids.CssLanguageServiceGuid,
                    "Unsupported language!");

                CacheExtensionSnippetInfo();

                Dictionary<string, List<SnippetInfo>> snippetListPerKind = new Dictionary<string, List<SnippetInfo>>();
                ISnippetKinds snippetKinds = SnippetKindManager.GetSnippetKinds(LanguageServiceGuid);

                if (_expansions != null) {
                    bool canceled = false;
                    List<SnippetInfo> combinedSnippetList = new List<SnippetInfo>(_expansions.Count); // avoid unnecessary re-allocations
                    snippetListPerKind[snippetKinds.GetCombinedSnippetKindName()] = combinedSnippetList;

                    foreach (VsExpansionEx expansion in _expansions) {
                        if (IsAbandoned) {
                            canceled = true;
                            break;
                        }

                        expansion.EnsurePopulated();
                        UpdateSnippet(expansion, snippetListPerKind, snippetKinds);
                    }

                    if (!canceled) {
                        _snippetInfoCache = snippetListPerKind;
                    }
                }
            }

            private void UpdateSnippet(
                VsExpansionEx expansion,
                Dictionary<string, List<SnippetInfo>> snippetListPerKind,
                ISnippetKinds snippetKinds) {
                // Path will be null for generated snippets imported via ISnippetProvider MEF extensions
                Debug.Assert(expansion.path == null || !_snippetFilePathToInfoMap.ContainsKey(expansion.path), "An extension has already been processed!");

                SnippetInfoList snippetInfoList = null;
                string path = expansion.path;

                UpdateSnippetInfo(ref snippetInfoList, path, expansion, snippetKinds);

                // Add snippet info to the appropriate snippet lists depending on
                // snippet kind.
                AddSnippetInfoPerKind(snippetInfoList, snippetListPerKind, snippetKinds);

                // Updated file-path-to-snippet-info-map
                if (path != null) {
                    _snippetFilePathToInfoMap.Add(path, snippetInfoList);
                }
            }

            void AddSnippetInfoPerKind(
                SnippetInfoList snippetInfoList,
                Dictionary<string, List<SnippetInfo>> snippetListPerKind,
                ISnippetKinds snippetKinds) {

                AddSnippetInfoPerKind(snippetKinds.GetCombinedSnippetKindName(), snippetInfoList, snippetListPerKind);

                if (snippetKinds.IsAllKind(snippetInfoList.Kind)) {
                    string[] uniqueSnippetKinds = snippetKinds.GetUniqueSnippetKindNames();
                    foreach (string snippetKind in uniqueSnippetKinds) {
                        AddSnippetInfoPerKind(snippetKind, snippetInfoList, snippetListPerKind);
                    }
                } else if (snippetInfoList.Kind != null) {
                    AddSnippetInfoPerKind(snippetInfoList.Kind, snippetInfoList, snippetListPerKind);
                }
            }

            private void AddSnippetInfoPerKind(
                string kind,
                SnippetInfoList snippetInfoList,
                Dictionary<string, List<SnippetInfo>> snippetListPerKind) {

                if (!snippetListPerKind.ContainsKey(kind)) {
                    snippetListPerKind[kind] = new List<SnippetInfo>();
                }

                snippetListPerKind[kind].AddRange(snippetInfoList);
            }

            void UpdateSnippetInfo(
                ref SnippetInfoList snippetInfoList,
                string path,
                VsExpansionEx expansion,
                ISnippetKinds snippetKinds) {

                Debug.Assert(snippetInfoList == null);
                snippetInfoList = new SnippetInfoList();
                snippetInfoList.Add(new SnippetInfo());

                // We already marshalled path, so just re-use it
                snippetInfoList.Path = path;

                // Marshall the rest of the data
                if (expansion.shortcut != null) {
                    snippetInfoList[0].Shortcut = expansion.shortcut;

                    AddKnownSnippetShortcut(snippetInfoList);
                }

                snippetInfoList[0].Description = expansion.description;
                snippetInfoList[0].Title = expansion.title;
                snippetInfoList[0].XmlDocument = expansion.document;

                try {
                    ParseSnippet(snippetInfoList, snippetKinds);
                } catch (Exception ex) {
                    // TODO: Write an error to Output Window
                    Debug.WriteLine("Unable to parse snippet " + snippetInfoList.Path + ", Exception: " + ex.Message);
                }
            }
            ...
            private void ParseSnippet(SnippetInfoList snippetInfoListToUpdate, ISnippetKinds snippetKinds) {
                ...
                if (snippetListPerKind != null) {
                    ...
                    if (snippetListPerKind.TryGetValue(kind, out snippetList)) {
                        ...
                        foreach (SnippetInfo snippetInfo in snippetList) {
                            ...
                            // Dev10 801400: if the snippet file defines project guids and the current project type guids is not a superset of the snippet guids
                            //   then don't add it
