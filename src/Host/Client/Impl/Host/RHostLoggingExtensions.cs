...
namespace Microsoft.R.Host.Client {
    internal static class RHostLoggingExtensions {
        public static void RHostProcessStarted(this IActionLog log, ProcessStartInfo psi) {
            ...
            log.WriteLineAsync(LogLevel.Minimal, MessageCategory.General, sb.ToString());
        }

        public static void RHostProcessExited(this IActionLog log) {
            log.WriteLineAsync(LogLevel.Minimal, MessageCategory.General, "R Host process exited");
        }

        public static void ConnectedToRHostWebSocket(this IActionLog log, string uri, int attempt) {
            log.WriteLineAsync(LogLevel.Minimal, MessageCategory.General, Invariant($"Connected to R Web Host socket at {uri} on attempt #{attempt}"));
        }

        public static void FailedToConnectToRHost(this IActionLog log) {
            log.WriteLineAsync(LogLevel.Minimal, MessageCategory.General, Invariant($"Timed out waiting for RHost to connect"));
        }

        public static void EnterRLoop(this IActionLog log, int depth) {
            log.WriteLineAsync(LogLevel.Normal, MessageCategory.General, Invariant($"Enter R loop, depth={depth}"));
        }

        public static void ExitRLoop(this IActionLog log, int depth) {
            log.WriteLineAsync(LogLevel.Normal, MessageCategory.General, Invariant($"Exit R loop, depth={depth}"));
        }

        public static void Request(this IActionLog log, string request, int depth) {
            log.WriteLineAsync(LogLevel.Traffic, MessageCategory.General, Invariant($"[Request,depth={depth}]:{request}"));
        }

        public static void Response(this IActionLog log, string response, int depth) {
            log.WriteLineAsync(LogLevel.Traffic, MessageCategory.General, Invariant($"[Response,depth={depth}]:{response}"));
        }
    }
}