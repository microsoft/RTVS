...
namespace Microsoft.R.Host.Client {
    public class DataTransferSession : IDisposable {
        ...
        /// </param>
        public async Task<IRBlobInfo> SendBytesAsync(byte[] data, CancellationToken cancellationToken, bool doCleanUp = true, IProgress<long> progress = null) {
            IRBlobInfo blob = null;
            using (RBlobStream blobStream = await RBlobStream.CreateAsync(_blobService))
            using (Stream ms = new MemoryStream(data)) {
                await ms.CopyToAsync(blobStream, cancellationToken, progress);
        ...
        /// true to add blob created upon transfer for cleanup on dispose, false to ignore it after transfer.
        /// </param>
        public async Task<IRBlobInfo> SendFileAsync(string filePath, CancellationToken cancellationToken, bool doCleanUp = true, IProgress<long> progress = null) {
            IRBlobInfo blob = null;
            using (RBlobStream blobStream = await RBlobStream.CreateAsync(_blobService))
            using (Stream fileStream = _fs.FileOpen(filePath, FileMode.Open)){
                await fileStream.CopyToAsync(blobStream, cancellationToken, progress);
                blob = blobStream.GetBlobInfo();
            }
        ...
        public async Task FetchFileAsync(IRBlobInfo blob, string filePath, CancellationToken cancellationToken, bool doCleanUp = true, IProgress<long> progress = null) {
            ...
            using (Stream fileStream = _fs.CreateFile(filePath)) {
                await blobStream.CopyToAsync(fileStream, cancellationToken, progress);
            }
        ...
        /// <param name="filePath">Path to the file where the retrieved data will be written.</param>
        /// <param name="doCleanUp">true to add blob upon transfer for cleanup on dispose, false to ignore it after transfer.</param>
        public async Task FetchAndDecompressFileAsync(IRBlobInfo blob, string filePath, CancellationToken cancellationToken, bool doCleanUp = true, IProgress<long> progress = null) {
            using (MemoryStream compressed = new MemoryStream(await FetchBytesAsync(blob, cancellationToken, false, progress)))
            using (ZipArchive archive = new ZipArchive(compressed, ZipArchiveMode.Read)) {
                ...
                using (Stream fileStream = _fs.CreateFile(filePath)) {
                    await decompressedStream.CopyToAsync(fileStream, cancellationToken, progress);
                }
            }
            if (doCleanUp) {
                _cleanup.Add(blob);
        ...
        /// <param name="blob">Blob from which the data is to be retrieved.</param>
        /// <param name="doCleanUp">true to add blob upon transfer for cleanup on dispose, false to ignore it after transfer.</param>
        public async Task<byte[]> FetchBytesAsync(IRBlobInfo blob, CancellationToken cancellationToken, bool doCleanUp = true, IProgress<long> progress = null) {
            byte[] data = null;
            ...
            using (MemoryStream ms = new MemoryStream((int)blobStream.Length)) {
                await blobStream.CopyToAsync(ms, cancellationToken, progress);
                data = ms.ToArray();
            }
        ...
        /// <param name="filePath">Path to the file where the retrieved data will be written.</param>
        /// <param name="doCleanUp">true to add blob upon transfer for cleanup on dispose, false to ignore it after transfer.</param>
        public async Task<byte[]> FetchAndDecompressBytesAsync(IRBlobInfo blob, CancellationToken cancellationToken, bool doCleanUp = true, IProgress<long> progress = null) {
            byte[] data = null;
            using (MemoryStream compressed = new MemoryStream(await FetchBytesAsync(blob, cancellationToken, false, progress)))
            using (ZipArchive archive = new ZipArchive(compressed, ZipArchiveMode.Read)) {
                var entry = archive.GetEntry("data");
                ...
                using (MemoryStream ms = new MemoryStream()) {
                    await decompressedStream.CopyToAsync(ms, cancellationToken, progress);
                    data = ms.ToArray();
                }
            }
        ...
        public async Task<string> CopyFileToRemoteTempAsync(string filePath, CancellationToken cancellationToken, bool doCleanUp = true, IProgress<long> progress = null) {
            string fileName = Path.GetFileName(filePath);
            var blobinfo = await SendFileAsync(filePath, cancellationToken, doCleanUp, progress);
