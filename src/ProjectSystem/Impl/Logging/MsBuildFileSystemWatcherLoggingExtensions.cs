...
namespace Microsoft.VisualStudio.ProjectSystem.FileSystemMirroring.Logging {
    internal static class MsBuildFileSystemWatcherLoggingExtensions {
        public static void WatcherStarting(this IActionLog log) {
            log.WriteLineAsync(LogLevel.Normal, MessageCategory.General, "MsBuildFileSystemWatcher starting");
        }

        public static void WatcherStarted(this IActionLog log) {
            log.WriteLineAsync(LogLevel.Normal, MessageCategory.General, "MsBuildFileSystemWatcher started");
        }

        public static void WatcherConsumeChangesScheduled(this IActionLog log) {
            log.WriteLineAsync(LogLevel.Normal, MessageCategory.General, "Consume file system changes scheduled");
        }

        public static void WatcherConsumeChangesStarted(this IActionLog log) {
            log.WriteLineAsync(LogLevel.Normal, MessageCategory.General, "File system changes consumer started");
        }

        public static void WatcherConsumeChangesFinished(this IActionLog log) {
            log.WriteLineAsync(LogLevel.Normal, MessageCategory.General, "File system changes consumer finished");
        }

        public static void WatcherApplyChange(this IActionLog log, string change) {
            log.WriteLineAsync(LogLevel.Normal, MessageCategory.General, Invariant($"Apply change: {change}"));
        }

        public static void WatcherApplyChangeFailed(this IActionLog log, string change, Exception exception) {
            log.WriteLineAsync(LogLevel.Normal, MessageCategory.Error, Invariant($"Failed to apply change '{change}':{exception}"));
        }

        public static void WatcherApplyRecoveryChange(this IActionLog log, string change) {
            log.WriteLineAsync(LogLevel.Normal, MessageCategory.General, Invariant($"Apply recovery change: {change}"));
        }

        public static void WatcherApplyRecoveryChangeFailed(this IActionLog log, string change, Exception exception) {
            log.WriteLineAsync(LogLevel.Normal, MessageCategory.Error, Invariant($"Failed to apply recovery change '{change}', closing watcher:{exception}"));
        }
        ...
        public static void WatcherChangesetSent(this IActionLog log, MsBuildFileSystemWatcher.Changeset changeset) {
            ...
            log.WriteAsync(LogLevel.Normal, MessageCategory.General, sb.ToString());
        }

        public static void ErrorInFileSystemWatcher(this IActionLog log, string watcherName, Exception e) {
            log.WriteAsync(LogLevel.Minimal, MessageCategory.Error, Invariant($"{watcherName} failed with exception:{e}"));
        }
